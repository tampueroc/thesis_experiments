...
from wildfire.model_latent_decoder.conditional_unet_01 import ConvBlock

TORCH = cast(Any, torch)


@dataclass(frozen=True)
class LandscapeBinaryMaskConditionalUNetConfig:
    image_channels: int = 1
    landscape_channels: int = 8
    ...
    def __post_init__(self) -> None:
        ...
        if self.landscape_channels <= 0:
            raise ValueError("landscape_channels must be > 0")
...
class LandscapeBinaryMaskConditionalUNetDecoder(nn.Module):
    def __init__(self, config: LandscapeBinaryMaskConditionalUNetConfig) -> None:
        super().__init__()
        self.config = config
        ...
        in_channels = config.image_channels + config.landscape_channels
        ...
        for skip_ch in skip_channels:
            self.upconvs.append(
                nn.ConvTranspose2d(decoder_in_channels, skip_ch, kernel_size=2, stride=2)
            )
    ...
    def forward(
        ...
        if landscape.ndim != 4:
            raise ValueError(f"landscape must be rank-4, got shape {tuple(landscape.shape)}")
        ...
        if prev_image.shape[0] != landscape.shape[0]:
            raise ValueError("prev_image and landscape batch size must match")
        if prev_image.shape[-2:] != landscape.shape[-2:]:
            raise ValueError("prev_image and landscape spatial size must match")

        x = TORCH.cat([prev_image, landscape], dim=1)
        skips: list[Any] = []
