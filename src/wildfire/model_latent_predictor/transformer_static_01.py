...
class StaticTransformerLatentPredictor(nn.Module):
    def __init__(self, config: StaticTransformerConfig) -> None:
        super().__init__()
        self.config = config
        ...
        static_input_dim = (config.static_num_dim * 2) + config.static_cat_dim
    ...
    def forward(self, z_in: Any, g_num: Any, g_num_mask: Any, g_cat: Any) -> Any:
        ...
        static_features = TORCH.cat([g_num, g_num_mask, g_cat.float()], dim=1)
        static_context = self.static_proj(static_features)
        encoded = self.encoder(x + static_context.unsqueeze(1))
