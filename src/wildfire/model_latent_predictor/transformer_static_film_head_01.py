...
class StaticFiLMHeadTransformerConfig:
    input_dim: int
    output_dim: int
    static_num_dim: int
    static_cat_dim: int
...
class StaticFiLMHeadTransformerLatentPredictor(nn.Module):
    def __init__(self, config: StaticFiLMHeadTransformerConfig) -> None:
        super().__init__()
        self.config = config
        ...
        self.static_proj = nn.Sequential(
            ...
            nn.Linear(config.static_hidden_dim, config.d_model * 2),
        )
    ...
    def forward(self, z_in: Any, g_num: Any, g_num_mask: Any, g_cat: Any) -> Any:
        ...
        film_params = self.static_proj(static_features)
        gamma, beta = TORCH.chunk(film_params, 2, dim=1)
        gamma = 1.0 + gamma
        modulated_state = (gamma * final_state) + beta
        return self.output_proj(modulated_state)
