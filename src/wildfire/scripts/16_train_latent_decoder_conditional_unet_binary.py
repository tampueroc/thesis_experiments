...
LOGGER = logging.getLogger("wildfire.train.latent_decoder_conditional_unet_binary")
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        parents=[pre_parser],
        description="Train 1-channel conditional U-Net latent decoder on aligned wildfire masks and embeddings.",
    )
    parser.add_argument(
        "--embeddings-root",
        type=Path,
    ...
    parser.add_argument("--family", default=cfg_value("family", "conditional_unet_binary"))
    ...
    parser.add_argument(
        "--hard-threshold",
        type=float,
        default=float(cfg_value("hard_threshold", 0.5)),
        help="Threshold used for hard-mask Dice/IoU evaluation.",
    )
    parser.add_argument(
        "--recon-log-interval",
        type=int,
...
def log_reconstruction_panel(
    *,
    ...
    wandb_image = wandb_handler.image(output_path, caption=f"{split_name} {tag} | threshold={hard_threshold}")
...
def main() -> None:
    ...
    LOGGER.info("hard_threshold=%.3f", args.hard_threshold)
    ...
    for epoch in range(1, args.epochs + 1):
        train_metrics = run_epoch(
            model, train_loader, bce_loss_fn, device, optimizer, args.dice_loss_weight, args.hard_threshold,
            show_progress=not args.no_progress, desc=f"train e{epoch:03d}",
        )
        with TORCH.no_grad():
            val_metrics = run_epoch(
                model, val_loader, bce_loss_fn, device, optimizer=None, dice_loss_weight=args.dice_loss_weight,
                hard_threshold=args.hard_threshold, show_progress=not args.no_progress, desc=f"val   e{epoch:03d}",
            )
        epoch_metrics = {
        ...
        LOGGER.info(
            "[epoch %03d] train/loss=%.6f val/loss=%.6f val/hard_dice=%.6f val/hard_iou=%.6f",
            epoch, train_metrics["loss"], val_metrics["loss"], val_metrics["hard_dice"], val_metrics["hard_iou"],
        )
    ...
    with TORCH.no_grad():
        train_eval = run_epoch(
            model, train_loader, bce_loss_fn, device, optimizer=None, dice_loss_weight=args.dice_loss_weight,
            hard_threshold=args.hard_threshold, show_progress=not args.no_progress, desc="eval train",
        )
        val_eval = run_epoch(
            model, val_loader, bce_loss_fn, device, optimizer=None, dice_loss_weight=args.dice_loss_weight,
            hard_threshold=args.hard_threshold, show_progress=not args.no_progress, desc="eval val",
        )
