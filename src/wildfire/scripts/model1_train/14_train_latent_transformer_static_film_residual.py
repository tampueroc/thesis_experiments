...
LOGGER = logging.getLogger("wildfire.train.transformer_static_film_head_residual_01")
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        parents=[pre_parser],
        description=(
            "Train static-FiLM-head residual transformer latent predictor on sequence "
            "embeddings with fire_id-level train/val/test split."
        ),
    )
    parser.add_argument(
        "--embeddings-root",
    ...
    parser.add_argument("--component", default=cfg_value("component", "latent_predictor"))
    parser.add_argument("--family", default=cfg_value("family", "transformer"))
    parser.add_argument("--variant", default=cfg_value("variant", "model_01_static_film_head_residual"))
    parser.add_argument(
        "--dataset-windowing",
    ...
    parser.add_argument("--dataset-history-min", type=int, default=int(cfg_value("dataset_history_min", 2)))
    parser.add_argument("--dataset-history-max", type=int, default=int(cfg_value("dataset_history_max", 10)))
    parser.add_argument(
        "--dataset-sampling",
        choices=["random_end_index", "enumerate_history_lengths"],
        default=cfg_value("dataset_sampling", "random_end_index"),
    )
    parser.add_argument("--dataset-stride", type=int, default=int(cfg_value("dataset_stride", 1)))
    ...
    parser.add_argument("--normalize-static-num", dest="normalize_static_num", action="store_true")
    parser.add_argument("--no-normalize-static-num", dest="normalize_static_num", action="store_false")
    parser.add_argument("--history", type=int, default=int(cfg_value("history", 5)))
    ...
    parser.add_argument("--dim-feedforward", type=int, default=int(cfg_value("dim_feedforward", 1024)))
    parser.add_argument("--static-hidden-dim", type=int, default=int(cfg_value("static_hidden_dim", 256)))
    parser.add_argument(
        "--static-cat-embed-dim",
        type=int,
        default=int(cfg_value("static_cat_embed_dim", 32)),
    )
    parser.add_argument("--norm-first", action="store_true", default=bool(cfg_value("norm_first", False)))
...
def static_num_indices(g_dim: int) -> np.ndarray:
    return np.asarray([i for i in range(g_dim) if i not in STATIC_CATEGORICAL_INDICES], dtype=np.int64)
...
def run_epoch(
    ...
    for batch in maybe_tqdm(loader, enabled=show_progress, desc=desc):
        ...
        z_delta_pred = predict_batch(model, batch, device)
        preds = z_prev + z_delta_pred
        ...
        for fire_id, z in z_by_fire.items():
            ...
            for target_t in range(history, timesteps - horizon + 1):
                ...
                for _step in range(horizon):
                    pred_delta = model(window_t, g_num_t, g_num_mask_t, g_cat_t)
                    z_prev = window_t[:, -1, :]
                    pred = z_prev + pred_delta
...
def main() -> None:
    ...
    if len(train_ds) == 0 or len(val_ds) == 0:
        raise RuntimeError(f"empty split after history={history_for_dataset}: train={len(train_ds)}, val={len(val_ds)}")
    ...
    model = StaticFiLMHeadTransformerLatentPredictor(config).to(device)
    ...
    LOGGER.info("static_num_normalization enabled=%s dims=%d", args.normalize_static_num, int(static_num_mean.shape[0]))
    LOGGER.info(
    ...
    with TORCH.no_grad():
        train_eval = run_epoch(
            model, train_loader, loss_fn, device, optimizer=None,
            show_progress=not args.no_progress, desc="eval train",
            variable_windowing=False, history_min=history_min, history_max=history_max,
        )
        val_eval = run_epoch(
            model, val_loader, loss_fn, device, optimizer=None,
            show_progress=not args.no_progress, desc="eval val",
            variable_windowing=False, history_min=history_min, history_max=history_max,
        )
