...
LOGGER = logging.getLogger("wildfire.eval.latent_decoder_conditional_unet_landscape_binary_predicted")
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        parents=[pre_parser],
        description="Evaluate a trained landscape-conditioned binary decoder checkpoint using predicted latent embeddings.",
    )
    ...
    parser.add_argument("--family", default=cfg_value("family", "conditional_unet_landscape_binary_predicted"))
...
def log_reconstruction_panel(
    *,
    model: Any,
    dataset: WildfireLandscapeDecoderDataset,
    device: Any,
    run_dir: Path,
    split_name: str,
    num_samples: int,
...
def select_sources(
    ...
    landscape_by_fire: dict[str, np.ndarray],
    keys: set[str],
) -> tuple[dict[str, np.ndarray], dict[str, list[Path]], dict[str, np.ndarray]]:
    return (
...
def main() -> int:
    ...
    if args.landscape_chw_dir is None:
        raise ValueError("--landscape-chw-dir is required")
    ...
    LOGGER.info("landscape_chw_dir=%s", args.landscape_chw_dir)

    predicted_z_by_fire, frames_by_fire = build_decoder_sources(model_dir=predicted_dir)
    landscape_by_fire = build_landscape_patch_sources(args.landscape_chw_dir, args.landscape_dir, frames_by_fire)
    ...
    common_ids = set(predicted_z_by_fire) & set(frames_by_fire) & set(landscape_by_fire)
    train_ids &= common_ids
    val_ids &= common_ids
    holdout_ids &= common_ids

    if not train_ids:
        raise RuntimeError("train_ids is empty after intersecting predicted embeddings, frames, and landscape")
    if args.normalize_landscape:
        landscape_mean, landscape_std = compute_landscape_channel_stats({k: landscape_by_fire[k] for k in train_ids})
    ...
    if args.max_sequences > 0:
        ...
        landscape_by_fire = {fire_id: landscape_by_fire[fire_id] for fire_id in limited_ids}
    ...
    train_ds = WildfireLandscapeDecoderDataset(
        *select_sources(predicted_z_by_fire, frames_by_fire, landscape_by_fire, train_ids),
        image_channels=1,
        binarize_masks=True,
    ...
    val_ds = WildfireLandscapeDecoderDataset(
        *select_sources(predicted_z_by_fire, frames_by_fire, landscape_by_fire, val_ids),
        image_channels=1,
        binarize_masks=True,
        mask_threshold=args.mask_threshold,
        normalize_embeddings=False,
    ...
    config = LandscapeBinaryMaskConditionalUNetConfig(**state["config"])
    model = LandscapeBinaryMaskConditionalUNetDecoder(config).to(device)
    ...
    wandb_handler = WandbHandler(
        ...
        config={
            ...
            "landscape_chw_dir": str(args.landscape_chw_dir),
            "landscape_channel_indices": args.landscape_channel_indices,
    ...
    summary = {
        ...
        "normalize_landscape": args.normalize_landscape,
        "landscape_mean": landscape_mean.tolist() if landscape_mean is not None else None,
        "landscape_std": landscape_std.tolist() if landscape_std is not None else None,
        "predictor_run_id": predictor_run_id,
        "predictor_variant": predictor_variant,
        "source_timestamp": source_timestamp,
        "device": str(device),
        ...
        "pipeline": {
            "prev_image": "real",
            "landscape": "real_sequence_constant",
