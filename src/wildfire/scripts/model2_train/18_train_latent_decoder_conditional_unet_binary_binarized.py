...
LOGGER = logging.getLogger("wildfire.train.latent_decoder_conditional_unet_binary_binarized")
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        parents=[pre_parser],
        description="Train binarized 1-channel conditional U-Net latent decoder.",
    )
    ...
    parser.add_argument("--family", default=cfg_value("family", "conditional_unet_binary_binarized"))
...
def select_sources(z_by_fire: dict[str, np.ndarray], frames_by_fire: dict[str, list[Path]], keys: set[str]) -> tuple[dict[str, np.ndarray], dict[str, list[Path]]]:
...
def log_reconstruction_panel(*, model: Any, dataset: WildfireDecoderDataset, device: Any, run_dir: Path, split_name: str, tag: str, num_samples: int, wandb_handler: WandbHandler, step: int | None, hard_threshold: float) -> Path | None:
    ...
    wandb_image = wandb_handler.image(output_path, caption=f"{split_name} {tag} | mask_threshold=binary, pred_threshold={hard_threshold}")
...
def main() -> int:
    ...
    for epoch in range(1, args.epochs + 1):
        train_metrics = run_epoch(model, train_loader, bce_loss_fn, device, optimizer, args.dice_loss_weight, args.hard_threshold, show_progress=not args.no_progress, desc=f"train e{epoch:03d}")
        with TORCH.no_grad():
            val_metrics = run_epoch(model, val_loader, bce_loss_fn, device, optimizer=None, dice_loss_weight=args.dice_loss_weight, hard_threshold=args.hard_threshold, show_progress=not args.no_progress, desc=f"val   e{epoch:03d}")
        epoch_metrics = {
    ...
    model.eval()
    with TORCH.no_grad():
        train_eval = run_epoch(model, train_loader, bce_loss_fn, device, optimizer=None, dice_loss_weight=args.dice_loss_weight, hard_threshold=args.hard_threshold, show_progress=not args.no_progress, desc="eval train")
        val_eval = run_epoch(model, val_loader, bce_loss_fn, device, optimizer=None, dice_loss_weight=args.dice_loss_weight, hard_threshold=args.hard_threshold, show_progress=not args.no_progress, desc="eval val")
