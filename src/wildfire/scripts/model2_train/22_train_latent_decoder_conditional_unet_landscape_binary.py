...
LOGGER = logging.getLogger("wildfire.train.latent_decoder_conditional_unet_landscape_binary")
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        parents=[pre_parser],
        description="Train landscape-conditioned binary conditional U-Net latent decoder.",
    )
    ...
    parser.add_argument("--conditioning-channels-per-embedding", type=int, default=int(cfg_value("conditioning_channels_per_embedding", 32)))
    parser.add_argument("--bottleneck-spatial-size", type=int, default=int(cfg_value("bottleneck_spatial_size", 25)))
    ...
    parser.add_argument("--recon-log-interval", type=int, default=int(cfg_value("recon_log_interval", 10)))
    ...
    parser.add_argument("--early-stop-mode", choices=["min", "max"], default=str(early_value("mode", "min")))
...
def split_fire_ids(fire_ids: list[str], train_ratio: float, val_ratio: float, seed: int) -> tuple[set[str], set[str], set[str]]:
    ...
    return (set(ordered_ids[:n_train]), set(ordered_ids[n_train : n_train + n_val]), set(ordered_ids[n_train + n_val :]))


def select_sources(z_by_fire: dict[str, np.ndarray], frames_by_fire: dict[str, list[Path]], landscape_by_fire: dict[str, np.ndarray], keys: set[str]) -> tuple[dict[str, np.ndarray], dict[str, list[Path]], dict[str, np.ndarray]]:
    return (
        {k: z_by_fire[k] for k in keys},
        {k: frames_by_fire[k] for k in keys},
        {k: landscape_by_fire[k] for k in keys},
    )
...
def run_epoch(model: Any, loader: Any, bce_loss_fn: Any, device: Any, optimizer: Any | None, dice_loss_weight: float, hard_threshold: float, show_progress: bool, desc: str) -> dict[str, float]:
    ...
    for batch in maybe_tqdm(loader, enabled=show_progress, desc=desc):
        ...
        logits = model(prev_image, landscape, prev_embedding, target_embedding)
...
def log_reconstruction_panel(*, model: Any, dataset: WildfireLandscapeDecoderDataset, device: Any, run_dir: Path, split_name: str, tag: str, num_samples: int, wandb_handler: WandbHandler, step: int | None, hard_threshold: float) -> Path | None:
    ...
    with TORCH.no_grad():
        for sample_idx in range(sample_count):
            ...
            landscape_tensor = cast(Any, sample["landscape"])
            ...
            landscape = landscape_tensor.unsqueeze(0).to(device)
            ...
            pred_logits = model(prev_image, landscape, prev_embedding, target_embedding)
    ...
    wandb_image = wandb_handler.image(output_path, caption=f"{split_name} {tag} | pred_threshold={hard_threshold}")
...
def main() -> int:
    ...
    val_ds = WildfireLandscapeDecoderDataset(
        *select_sources(z_by_fire, frames_by_fire, landscape_by_fire, val_ids),
        image_channels=1,
        binarize_masks=True,
        mask_threshold=args.mask_threshold,
        normalize_embeddings=args.normalize_embeddings,
    ...
    landscape = np.asarray(sample["landscape"])
    prev_embedding = np.asarray(sample["prev_embedding"])
    expected_bottleneck = int(prev_image.shape[-1]) // (2 ** args.num_pool_layers)
    if args.bottleneck_spatial_size != expected_bottleneck:
        raise ValueError(
            f"bottleneck_spatial_size={args.bottleneck_spatial_size} does not match image_size={prev_image.shape[-1]} "
    ...
    wandb_handler = WandbHandler(
        ...
        run_name=args.wandb_run_name if args.wandb_run_name else run_id,
        output_dir=run_dir,
        tags=wandb_tags,
    ...
    for epoch in range(1, args.epochs + 1):
        train_metrics = run_epoch(model, train_loader, bce_loss_fn, device, optimizer, args.dice_loss_weight, args.hard_threshold, not args.no_progress, f"train e{epoch:03d}")
        with TORCH.no_grad():
            val_metrics = run_epoch(model, val_loader, bce_loss_fn, device, None, args.dice_loss_weight, args.hard_threshold, not args.no_progress, f"val   e{epoch:03d}")
        epoch_metrics = {
            "train/loss": train_metrics["loss"],
            "train/bce": train_metrics["bce"],
            "train/dice_loss": train_metrics["dice_loss"],
            "train/soft_dice": train_metrics["soft_dice"],
            "train/soft_iou": train_metrics["soft_iou"],
            "train/hard_dice": train_metrics["hard_dice"],
            "train/hard_iou": train_metrics["hard_iou"],
            "val/loss": val_metrics["loss"],
            "val/bce": val_metrics["bce"],
            "val/dice_loss": val_metrics["dice_loss"],
            "val/soft_dice": val_metrics["soft_dice"],
            "val/soft_iou": val_metrics["soft_iou"],
            "val/hard_dice": val_metrics["hard_dice"],
            "val/hard_iou": val_metrics["hard_iou"],
        }
        monitor_value = float(epoch_metrics[args.early_stop_metric])
        improved, should_stop = early_stopper.update(monitor_value)
        LOGGER.info("[epoch %03d] train/loss=%.6f val/loss=%.6f val/hard_dice=%.6f val/hard_iou=%.6f", epoch, train_metrics["loss"], val_metrics["loss"], val_metrics["hard_dice"], val_metrics["hard_iou"])
        ...
        if args.recon_log_interval > 0 and (epoch == 1 or epoch % args.recon_log_interval == 0):
            log_reconstruction_panel(model=model, dataset=val_ds, device=device, run_dir=run_dir, split_name="val", tag=f"epoch_{epoch:03d}", num_samples=args.recon_log_samples, wandb_handler=wandb_handler, step=epoch, hard_threshold=args.hard_threshold)
    ...
    model.eval()
    with TORCH.no_grad():
        train_eval = run_epoch(model, train_loader, bce_loss_fn, device, None, args.dice_loss_weight, args.hard_threshold, not args.no_progress, "eval train")
        val_eval = run_epoch(model, val_loader, bce_loss_fn, device, None, args.dice_loss_weight, args.hard_threshold, not args.no_progress, "eval val")
    final_val_panel = log_reconstruction_panel(model=model, dataset=val_ds, device=device, run_dir=run_dir, split_name="val", tag="final", num_samples=args.recon_log_samples, wandb_handler=wandb_handler, step=None, hard_threshold=args.hard_threshold)
    summary = {
        "run_id": run_id,
        "component": args.component,
        "family": args.family,
        ...
        "qualitative": {"val_recon_panel": str(final_val_panel) if final_val_panel is not None else ""},
    }
